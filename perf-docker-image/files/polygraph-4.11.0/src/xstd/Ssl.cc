...
SslCtx::SslCtx(SslProtocol protocol, const String &cipher): theCtx(0) {
	...
	switch(protocol) {
		...
		case TLSv1:
			...
			break;
		case TLSv1_1:
			theCtx = ::SSL_CTX_new(::TLSv1_1_method());
			break;
		case TLSv1_2:
			theCtx = ::SSL_CTX_new(::TLSv1_2_method());
			break;
		case SSLv3:
...
bool Ssl::sni(const char *name) {
#if OPENSSL_ENABLED
    // OpenSSL defines SSL_CTRL_SET_TLSEXT_HOSTNAME to signal SNI support
    #ifdef SSL_CTRL_SET_TLSEXT_HOSTNAME
        return SSL_set_tlsext_host_name(theConn, name) == 1;
    #endif
#endif
    return false;
}

bool Ssl::SniSupported() {
#if OPENSSL_ENABLED
    #ifdef SSL_CTRL_SET_TLSEXT_HOSTNAME
        return true;
    #endif
