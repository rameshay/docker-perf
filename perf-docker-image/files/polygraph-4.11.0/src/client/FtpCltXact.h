...
class FtpCltXact: public CltXact {
	...
	protected:
		...
		Error checkDataChannel();

		// future support for data-carrying commands like LIST will make this logic less static
		bool dataCommand(const FtpReq::Command cmd) const { return cmd == FtpReq::frcRETR || cmd == FtpReq::frcSTOR; }
	...
	protected:
		...
		// We sent this command and started listening for an active data connection.
		// The server has already told us (via control connection) about this command
		// completion, but the corresponding data connection has not reached us yet.
		FtpReq::Command theActiveDataWaitingCmd;
