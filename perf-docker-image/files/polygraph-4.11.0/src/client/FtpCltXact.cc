...
void FtpCltXact::reset() {
	...
	theActiveDataWaitingCmd = FtpReq::frcUnknown;
	theDataConn.reset();
...
Error FtpCltXact::interpretReply() {
	...
	switch (theRep.code()) {
		...
		case 150: // Opening data connection.
			err = checkDataChannel();
			if (!err && theReqCmd != FtpReq::frcUnknown)
				err = transition(theReqCmd, FtpReq::frcUnknown);
			break;
		case 226: // Operation completed
			err = checkDataChannel(); // in case an optional(?) 125 or 150 was not received
			if (!err)
...
Error FtpCltXact::checkDataChannel() {
	Error err = 0;
	if (dataCommand(theReqCmd)) {
		if (theDataChannelState == dcsConnected)
			err = kickDataChannel();
		else if (theDataChannelState == dcsListening)
			theActiveDataWaitingCmd = theReqCmd;
	}
	return err;
}
...
//Comment << here << "starting to read data; new data state: " << theDataChannelState << endc;
	const FtpReq::Command reqCmd = dataCommand(theReqCmd) ?
		theReqCmd : theActiveDataWaitingCmd;
	if (reqCmd == FtpReq::frcRETR)
		theDataConn.theRd.start(this);
	else if (ShouldUs(reqCmd == FtpReq::frcSTOR))
...
//Comment << theSock.fd() << " accepted " << theDataConn.sock().fd() << endc;
	...
	if (dataCommand(theReqCmd) || dataCommand(theActiveDataWaitingCmd))
		kickDataChannel();
	else
		finish(errOther);
}
