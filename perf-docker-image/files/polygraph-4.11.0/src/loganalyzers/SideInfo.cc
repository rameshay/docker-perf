...
void SideInfo::compileStats(BlobDb &db, const PhaseInfo &phase, const Scope &scope) {
	...
	cmplProtoConnLevelTable(db, phase, scope);
...
void SideInfo::cmplConnLevelTable(BlobDb &db, const PhaseInfo &phase, const Scope &scope) {
...

	{
		...
		XmlTextTag<XmlParagraph> p1;
		p1.buf() << "The concurrent TCP connection level table shows the "
...
void SideInfo::cmplConnLevelFigure(BlobDb &db, ReportBlob &blob, const Scope &scope) {
	...
	LevelTraceFig fig;
	fig.configure("conn.level.trace" + scope, "concurrent TCP connection level trace");
	fig.stats(&stex1, &phase(scope));
	fig.compareWith(&stex2);
...
// TODO: Also report transaction level, which is different than connection level
// when there are concurrent transactions on a single connection (e.g., HTTP/1
// pipelining and HTTP/2 streaming) and/or idle connections. See also: xact.level.table
void SideInfo::cmplProtoConnLevelTable(BlobDb &db, const PhaseInfo &phase, const Scope &scope) {
	ReportBlob blob("proto.conn.level.table" + scope, "protocol connection levels");
	blob << XmlAttr("vprimitive", "Protocol connection level table");
	...
	{
		XmlTableRec tr;
		tr << XmlTableHeading("Protocol", 1, 2);
		tr << XmlTableHeading("Number of times", 2, 1);
		tr << XmlTableHeading("Mean concurrency level", 1, 2);
		table << tr;
	}
	...
	{
		XmlTableRec tr;
		tr << XmlTableHeading("opened");
		tr << XmlTableHeading("closed");
		table << tr;
	}

	cmplLevelTableRec(db, "ssl.conn.", "SSL", stats.theSslStat.connLevel(), scope, table);
	cmplLevelTableRec(db, "ftp.conn.", "FTP", stats.theFtpStat.connLevel(), scope, table);
	cmplLevelTableRec(db, "socks.conn.", "SOCKS", stats.theSocksStat.connLevel(), scope, table);
...

	{
		...
		XmlTextTag<XmlParagraph> p1;
		p1.buf() << "The protocol connection level table table shows "
			"the number of open connections used by various transfer protocols. "
			"The exact notion of a \"connection\" is protocol-dependent.";
		descr << p1;

		XmlTag list("ul");

		list << XmlText(
			"SSL: Roughly speaking, an SSL connection is a time period from "
			"the start of an SSL/TLS handshake to the end of SSL/TLS shutdown. "
			"Please note that a TCP connection may be used for plain HTTP "
			"traffic (e.g., CONNECT requests) before the handshake. Moreover, "
			"with HTTPS proxy and/or HTTP/2 support, a single TCP connection may "
			"carry multiple concurrent SSL connections. Thus, even in an "
			"HTTPS-only test, the number of concurrent SSL connections may be "
			"lower than or may exceed the number of concurrent TCP connections!"
		);

		list << XmlText(
			"FTP: An FTP connection is a TCP connection designated for "
			"carrying either FTP control messages or FTP data bytes. "
			"Proxied FTP-over-HTTP transactions do not use FTP connections."
		);

		list << XmlText(
			"SOCKS: A SOCKS connection is a TCP connection "
			"routed through a SOCKS proxy. SOCKS connections may carry HTTP or "
			"FTP transactions as well as SSL connections."
		);
	...
	// XXX: move:
	cmplProtoConnLevelFigure(db, blob, scope);

	db << blob;
}

void SideInfo::cmplProtoConnLevelFigure(BlobDb &db, ReportBlob &blob, const Scope &scope) {
	LevelStex stex1("ssl.conn", "SSL", &StatIntvlRec::theSslStat, &ProtoIntvlStat::connLevel);
	LevelStex stex2("ftp.conn", "FTP", &StatIntvlRec::theFtpStat, &ProtoIntvlStat::connLevel);
	LevelStex stex3("socks.conn", "SOCKS", &StatIntvlRec::theSocksStat, &ProtoIntvlStat::connLevel);
	LevelTraceFig fig;
	fig.configure("proto.conn.level.trace" + scope, "concurrent protocol connection level trace");
	fig.stats(&stex1, &phase(scope));
	fig.compareWith(&stex2);
	fig.compareWith(&stex3);
	fig.globalStart(theTest->startTime());
...
void SideInfo::cmplLevelTableRec(BlobDb &db, const String &pfx, const String &state, const LevelStat &stats, const Scope &scope, XmlTable &table) {
	// TODO:
	// if (!stats.known())
	// ... record missing MeasBlobs and return
