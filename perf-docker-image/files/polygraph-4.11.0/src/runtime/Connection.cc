...
bool Connection::LayeredSsl::shutdown(int &res) {
	int i = lastLayer;
	do {
		if (!theSsl[i]->inInit() && !theSsl[i]->shutdown(res))
			return false;
	} while (i--);
	return true;
}
...
void Connection::reset() {
	...
	theProxy = theSocksProxy = theTunnel = theSni = NetAddr();
...
bool Connection::connect(const NetAddr &addr, const SockOpt &opt, PortMgr *aPortMgr, const NetAddr &socksProxy, const NetAddr &proxy) {
	theAddr = addr;
...
void Connection::sslStart(int role, const bool toProxy) {
	...
	if (sslSession) {
		if (!ssl->resumeSession(sslSession) &&
			SslErrors::Report(Comment, Here, errSslSessionResume, logCat())) {
	...
	// If we are establishing an SSL connection with a proxy, theSni address is
	// irrelevant, but its known() status indicates whether we should send SNI.
	const NetAddr &sni = toProxy && theSni.known() ? theProxy : theSni;
	if (Ssl::SniSupported() && sni.known() && sni.isDomainName() && !ssl->sni(sni.addrA().cstr()))
		sslError(0, toProxy ? "set proxy SNI" : "set origin SNI");
...
void Connection::sslError(int err, const char *operation) {
	if (SslErrors::Report(Comment, Here, errSslIo, logCat())) {
		Comment << "error details: SSL " << operation
