...
SslWrap::SslWrap(): canShare(false), theProtocolSel(0), theRsaKeySizeSel(0),
	theCertificatesIdx(0),
	theCipherSel(0),
	theResumpProb(-1),
	theSessionCacheSize(-1),
	theCompression(-1),
	theSniProb(-1),
	doVerifyPeerCertificate(true),
	doGenerateCertificates(false),
	isGenerageCertificatesSet(false) {
...
void SslWrap::configure(const SslWrapSym &cfg) {
	...
	if (cfg.sni(theSniProb)) {
		if (theSniProb < 0)
			theSniProb = 0;
		else if (theSniProb > 1)
			theSniProb = 1;

		if (theSniProb > 0 && !Ssl::SniSupported()) {
			Comment << cfg.loc() << "error: TLS SNI requested, but Polygraph was built with an OpenSSL library version that does not support SNI."
				<< endc << xexit;
		}
	} else {
		// by default, send SNI if possible
		theSniProb = Ssl::SniSupported() ? 1 : 0;
	}
...
void SslWrap::configureProtocols(const SslWrapSym &cfg) {
	...
	if (!sidf.count()) {
		...
		sidf.add("TLSv1.1", SslCtx::TLSv1_1);
		sidf.add("TLSv1.2", SslCtx::TLSv1_2);
...
bool SslWrap::supportSni() const {
	static RndGen *rng = LclRndGen("SslWrap::sni");
	return theSniProb > 0 ? rng->event(theSniProb) : false;
}
...
SslCtx *SslWrap::makeClientCtx(const NetAddr &addr) const {
	...
	SslErrors::Print(Comment << "error: failed to load root certificate from " <<
		theRootCertificate << std::endl) << endc;
...
SslCtx *SslWrap::makeCtx(const bool isServer) const {
	...
	if (!ctx->usePrivateKeyFile(theChainPem)) {
		SslErrors::Print(Comment << "error: failed to use private key from " <<
			theChainPem << std::endl) << endc << xexit;
	}

	if (!ctx->useCertificateChainFile(theChainPem)) {
		SslErrors::Print(Comment << "error: failed to use certificate chain from " <<
			theChainPem << std::endl) << endc << xexit;
	}
...
String SslCommand::calcKey(const String &cmd) const {
	...
	const int workerId = TheOpts.theWorkerId;
	alg.update(reinterpret_cast<const char*>(&workerId), sizeof(workerId));
