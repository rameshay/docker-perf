...
ostream &Oid2UrlPath(const ObjId &oid, ostream &os) {
	...
	if (ccfg) {
		const int seedExt = GlbPermut(oid.hash(), rndContentExt);
		if (const String &ext = ccfg->url_ext(seedExt))
			os << ext;

		const int seedQry = GlbPermut(oid.hash(), rndContentQry);
		if (const String &qry = ccfg->url_qry(seedQry))
			os << qry;
	}
	return os;
}
