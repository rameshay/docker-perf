...
const String MimeHeaderSym::TransactionIdMacro = "${transaction_id}";

static const String strName = "name";
static const String strValue = "value";

MimeHeaderSym::MimeHeaderSym():
	RecSym(TheType, new PglRec),
	checkedMacros_(false),
	hasMacros_(false) {
...
MimeHeaderSym::MimeHeaderSym(const String &aType, PglRec *aRec):
	RecSym(aType, aRec),
	// for simplicity sake, (re-)compute these when needed
	checkedMacros_(false),
	hasMacros_(false) {
}
...
bool MimeHeaderSym::hasMacros() const {
	if (!checkedMacros_) {
		checkedMacros_ = true;
		if (const String *val = value())
			hasMacros_ = HasSomeMacros(*val, TransactionIdMacro);
		else
			hasMacros_ = false; // paranoid: ought to be false by default
	}
	return hasMacros_;
}
