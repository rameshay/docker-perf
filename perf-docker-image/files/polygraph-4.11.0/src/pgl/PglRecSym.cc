...
// TODO: Add more set*() methods and convert old code to use them.
void RecSym::setString(const String &name, const String &value) {
	SynSymTblItem *item = 0;
	Assert(theRec->find(name, item));
	delete item->sym(); // TODO: SynSymTblItem::sym(...) should do that
	item->sym(new StringSym(value));
}

void RecSym::absorbArraySym(const String &name, ArraySym *value) {
	SynSymTblItem *item = 0;
	Assert(rec()->find(name, item));
	delete item->sym(); // TODO: SynSymTblItem::sym(...) should do that
	item->sym(value);
}
