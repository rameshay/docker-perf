...
bool ObjId::parse(const char *&buf, const char *end) {
	...
	if (const char *oid = StrBoundChr(buf, '_', end)) {
		const char *oidBuf = oid + 1;
		const int intSize = 16;
		if (end - oidBuf > intSize && isxdigit(oidBuf[intSize])) {
			static char intBuf[intSize + 1];
			memcpy(intBuf, oidBuf, intSize);
			intBuf[intSize] = 0;
			const char *endIntBuf = intBuf;
			isInt64(intBuf, theName, &endIntBuf, 16);
			buf += endIntBuf - intBuf;
		} else {
			isInt64(oidBuf, theName, &buf, 16);
		}
	}
