...
void DnsMgr::configure(const DnsResolverSym *cfg) {
	...
	if (const ArraySym * const zonesSym = cfg->zones()) {
		PtrArray<AddrMapSym*> singleFamilyZones;

		// convert each (potentially) multifamily zone into a single family one
		// and also clear any address ports
		PtrArray<AddrMapSym*> multiFamilyZones;
		zonesSym->exportA(multiFamilyZones);
		singleFamilyZones.stretch(multiFamilyZones.count());
		const int family = (theType == DnsMsg::typeA) ? AF_INET : AF_INET6;
		for (int i = 0; i < multiFamilyZones.count(); ++i) {
			AddrMapSym *zone = multiFamilyZones[i]->adapt(family, true);
			if (zone->usable()) // ignore zones that have no family records
				singleFamilyZones.append(zone);
			else
				delete zone;
		}

		thePreloadedAddresses.configure(singleFamilyZones);
	}
}
...
DnsMgr::InstantLookupResult DnsMgr::instantLookup(const NetAddr &addr, NetAddr &ip) const {
	...
	if (thePreloadedAddresses.nameCount() > 0) {
		// XXX: Creating addresses just to remove ports is slow.
		// TODO: Teach AddrMap to optionally(?) ignore ports instead.
		const NetAddr domain(addr.addrA(), -1);
		int nameIdx = -1;
		if (thePreloadedAddresses.find(domain, nameIdx)) {
			ip = thePreloadedAddresses.addrAt(nameIdx, 0);
			return dnsZoneHit;
		}
	}
